...
public final class ByteQuadsCanonicalizer
{
  ...
  public ByteQuadsCanonicalizer makeChild(int flags)
  {
    return new ByteQuadsCanonicalizer(this, JsonFactory.Feature.INTERN_FIELD_NAMES
      .enabledIn(flags), _seed, JsonFactory.Feature.FAIL_ON_SYMBOL_HASH_OVERFLOW
      
      .enabledIn(flags), 
      (TableInfo)_tableInfo.get());
  }
  ...
  {
    int count = 0;
    int offset = 3; for (int end = _secondaryStart; offset < end; offset += 4) {
  ...
  {
    int count = 0;
    int offset = 3; for (int end = _hashSize << 3; offset < end; offset += 4) {
      if (_hashArea[offset] != 0) {
        count++;
      }
    }
  ...
  public String toString()
  {
    ...
    return String.format("[%s: size=%d, hashSize=%d, %d/%d/%d/%d pri/sec/ter/spill (=%s), total:%d]", new Object[] {
      getClass().getName(), Integer.valueOf(_count), Integer.valueOf(_hashSize), 
  ...
  public String addName(String name, int[] q, int qlen)
  {
    ...
    {
    case 1: 
      int offset = _findOffsetForAdd(calcHash(q[0]));
    ...
    case 2: 
      int offset = _findOffsetForAdd(calcHash(q[0], q[1]));
      _hashArea[offset] = q[0];
    ...
    case 3: 
      int offset = _findOffsetForAdd(calcHash(q[0], q[1], q[2]));
      _hashArea[offset] = q[0];
      _hashArea[(offset + 1)] = q[1];
  ...
  private void rehash()
  {
    ...
    int copyCount = 0;
    int[] q = new int[16];
    int offset = 0; for (int end = oldEnd; offset < end; offset += 4) {
