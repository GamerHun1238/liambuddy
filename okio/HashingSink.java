  ...
  extends ForwardingSink
{
  ...
  public void write(Buffer source, long byteCount) throws IOException {
    ...
    long hashedCount = 0L;
    for (Segment s = head; hashedCount < byteCount; s = next) {
      int toHash = (int)Math.min(byteCount - hashedCount, limit - pos);
      if (messageDigest != null) {
        messageDigest.update(data, pos, toHash);
      } else {
        mac.update(data, pos, toHash);
      }
      hashedCount += toHash;
    }
